 ...
 * 웅이가 준비한 음식의 양을 칼로리가 적은 순서대로 나타내는 정수 배열 food가 주어졌을 때, 대회를 위한 음식의 배치를 나타내는 문자열을 return 하는 solution 함수를 완성해주세요.
 */
...
class Solution {
    public String solution(int[] food) {
        ...
        for(int i = 1; i < food.length; i++){          
            int num = food[i]/2; //홀수상태이면 어차피 1개는 사용못하므로 2로 나누어준다.
            sb.append(String.valueOf(i).repeat(num)); //StringBuffer를 사용하여 응답이 빠르게 int값을 String.valueOf로 타입변환 repeat는 넣은 수만큼 반복해서 추가된다.
        }
        String front = sb+"0";
        return front+sb.reverse(); //reverse()메서드를 통하여 단어를 뒤집어준다.
    }
}